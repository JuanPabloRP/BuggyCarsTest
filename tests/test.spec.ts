...
LOS TEST CORRERAN SOLO EN CHROMIUM DEBIDO A QUE LA PAGINA BLOQUEA TANTAS SOLICITUDES A LA HORA
DE ACTUALIZAR DATOS COMO LA CONTRASEÑA U OTROS DATOS DEL PERFIL DEL USUARIO
...
test('votacion solo para usuarios autenticados', async ({ page }) => {
	await page.goto('https://buggy.justtestit.org/');
	await page.getByPlaceholder('Login').fill('testuser7808');
	await page.locator('input[name="password"]').click();
